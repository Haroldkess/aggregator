...
class VendorController extends GetxController {
  ...
  var vendorRating = RatingModel().obs;
  ...
  Future vendorRatingList(int id) async {
    isLoadRating.value = true;
    late String token;
    //  String id = UserController.instance.user.value.id.toString();
    //update();
    var url = Api.baseUrl + Api.getVendorListRatings + "$id?start=1&end=4";
    await KeyStore.getToken().then((element) {
      token = element!;
    });
    consoleLog(token);
    try {
      ...
      var save = RatingModel.fromJson(jsonDecode(responseData));
      vendorRating.value = save;
      update();
    } catch (e) {
      log(e.toString());
    }
